    ...
    while(accountIterator.hasNext()) {
        ...
        while(accountLabelIterator.hasNext()) {
            ...
            if (label.getName() == LABEL_NAME) {
                ...
                campaignsNumber += getCampaignDataAndPush(campaignsIterator, campaignsToCheck, EXCLUDED, THRESHOLD, yesterday)
                ...
                campaignsNumber += getCampaignDataAndPush(performanceMaxCampaignsIterator, campaignsToCheck, EXCLUDED, THRESHOLD, yesterday)
            }
        }
    }
...
function getCampaignDataAndPush(campaignTypeIterator, campaignsToCheck, EXCLUDED, THRESHOLD, yesterday) {
            ...
            if (!ifIncludes(campaign.getName(), EXCLUDED)) {
                ...
                if (campaignDailyCost >= 1) {
                    if (!checkDailyCost(campaignDailyBudget, campaignDailyCost, THRESHOLD)) {
                        campaignsToCheck.push(createCampaignObject(campaignDailyBudget, campaignDailyCost, campaign.getName()))
                    }
                }
            }
        }
...
function createCampaignObject(campaignDailyBudget, campaignDailyCost, campaignName) {
...
function displayCampaignObjects(campaignsToCheck) {
    ...
    campaignsToCheck.forEach(campaignObject => {
     ...
     USTALONY BUDŻET W DNIU ${YESTERDAY}:      ${campaignObject.campaignDailyBudget}, 
